...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_model(self):
        if self.config.actor.actor_type == 'cce':
            from verl.workers.actor.cce_actor import CCE_DP_PPOActor as Actor
        else:
            from verl.workers.actor.dp_actor import DataParallelPPOActor as Actor
        ...
        if self._is_actor:
            ...
            self.actor = Actor(config=self.config.actor,
        ...
        if self._is_ref:
            ...
            self.ref_policy = Actor(config=self.config.ref, actor_module=self.ref_module_fsdp)
